...
def process(prevSkyLineName: str,
            rouOstFile: str,
            nonOHFile: str,
            outfile: str):
    ...
    for refLine in rouOstDict.values():
        if refLine.wavelength in lineDict:
            print(f'WARNING: Wavelength {refLine.wavelength} '
                  'is already added. Skipping MERGED line')
    ...
    #     # if line.intensity < 0.1 or line.wavelength < 630.00 or
    #     #     line.source == ReferenceLineSource.OSTERBROCK97:
    ...
    # Remove all lines marked as MERGED temporarily
    # until DRP updated to support these
    filteredLineList = []
    for line in linelist:
            ...
            if len(commentedLines) == 2:
                if lineStripped.endswith('[5]*'):
                    ...
                    if wavelength in lineDict:
                        raise ValueError(f'Wavelength {wavelength} '
                                         'is already added.')
                    lineDict[refLine.wavelength] = refLine
            commentedLines = []
    return lineDict
...
def combineExplicitGroups(linelist):
    ...
    Note some strings are too long to fit into an afw.table
    when processing (limit 128 char).
    ...
    for idx, wD in enumerate(wavelengthData):
        ...
        if len(transition) >= 128:
            raise ValueError(f'entry {idx} has a transition of length > 128 '
                             f'({len(transition)})')
        if ' ' in transition:
...
def main():
    inputfilename = os.path.join('derived-data',
                                 'skyLines-1fc7b67.txt')
    rouOstFileName = os.path.join('derived-data',
                                  'rousselot-osterbrock-combined.txt')
